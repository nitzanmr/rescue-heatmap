...
USAGE
  ...
  python3 ops/scrape/colombiatebusca.py --out ... --details --category terremoto
"""
...
def parse_listing(page_html: str) -> list[dict]:
    ...
    for raw in CARD_RE.findall(page_html):
        pid = re.search(r"\?person=([0-9a-f-]{36})", raw)
...
def main() -> int:
    ...
    ap.add_argument("--details", action="store_true", help="also fetch one page per person")
